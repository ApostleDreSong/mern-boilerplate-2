...
describe('<Masonary />', () => {
  it('renders the styled component <MasonryContainer />', () => {
    const wrapper = mount(<Masonry>{fourChildren}</Masonry>);
  ...
  it('when window.width >= 1200px and # children >= 3; renders 3 <MasonryColumn /> and places children in correct columns', () => {
    ...
    expect(wrapper
      .find('MasonryColumn')
      .at(0)
      .find('Billboard')).to.have.length(2);
    expect(wrapper
      .find('MasonryColumn')
      .at(1)
      .find('Billboard')).to.have.length(1);
    expect(wrapper
      .find('MasonryColumn')
      .at(2)
      .find('Billboard')).to.have.length(1);
  ...
  it('when window.width >= 1200px and # children == 1; renders 1 <MasonryColumn /> and places children in this column', () => {
    window.resizeTo(1920, 1080);
    const wrapper = mount(<Masonry>{oneChild}</Masonry>);
    expect(wrapper.find('MasonryColumn')).to.have.length(1);
    expect(wrapper
      .find('MasonryColumn')
      .at(0)
